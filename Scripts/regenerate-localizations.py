...
PRO_PLUS_IPHONE_STRINGS = {
    ...
    # v2.0.1 (2026-05-13): Xcode's auto-extractor materialised these
    # %@-form keys parallel to the existing \(varname) entries when
    # the SplynekCompanion target reused the same Text() interpolations.
    # The catalog audit flags them as missing across all non-en locales.
    "%@ scanned": {
        "pt-PT": "%@ analisadas",
        "es": "%@ escaneadas",
        "fr": "%@ analysées",
    ...
    # v2.0.1 batch: 57 strings auto-extracted by Xcode that weren't
    # yet in the .py source-of-truth.  Most are %@-form mirrors of
    # existing \(varname) entries; punctuation/symbol passthroughs
    # keep the same value across locales.
    "\"%@\" → %@ step%@, %@ require confirmation.": {
        "pt-PT": "\"%1$@\" → %2$@ passo%3$@, %4$@ requerem confirmação.",
        "es": "\"%1$@\" → %2$@ paso%3$@, %4$@ requieren confirmación.",
        "fr": "\"%1$@\" → %2$@ étape%3$@, %4$@ nécessitent confirmation.",
        "de": "„%1$@\" → %2$@ Schritt%3$@, %4$@ erfordern Bestätigung.",
        "it": "\"%1$@\" → %2$@ passaggi%3$@, %4$@ richiedono conferma.",
    },
    "%@": {"pt-PT": "%@", "es": "%@", "fr": "%@", "de": "%@", "it": "%@"},
    "%@ / %@ apps": {
        "pt-PT": "%1$@ / %2$@ apps",
    ...
    "%@ apps without a known update source": {
        "pt-PT": "%@ apps sem fonte de atualização conhecida",
        "es": "%@ apps sin fuente de actualización conocida",
        "fr": "%@ applications sans source de mise à jour connue",
        "de": "%@ Apps ohne bekannte Update-Quelle",
        "it": "%@ app senza fonte di aggiornamento nota",
    },
    "%@ chunks": {
        "pt-PT": "%@ blocos",
        "es": "%@ trozos",
    ...
    "%@ file%@ hidden from fleet.": {
        "pt-PT": "%1$@ ficheiro%2$@ oculto%2$@ da frota.",
        "es": "%1$@ archivo%2$@ oculto%2$@ de la flota.",
        "fr": "%1$@ fichier%2$@ masqué%2$@ de la flotte.",
        "de": "%1$@ Datei%2$@ vor Flotte versteckt.",
        "it": "%1$@ file%2$@ nascost%2$@ alla flotta.",
    },
    "%@ step%@. Each one runs only when you confirm. Nothing is deleted; the original app stays put unless you uninstall it manually.": {
        "pt-PT": "%1$@ passo%2$@. Cada um corre apenas quando confirmas. Nada é apagado; a app original mantém-se a menos que a desinstales manualmente.",
        "es": "%1$@ paso%2$@. Cada uno se ejecuta solo cuando lo confirmas. Nada se elimina; la app original permanece a menos que la desinstales manualmente.",
        "fr": "%1$@ étape%2$@. Chacune ne s'exécute que sur confirmation. Rien n'est supprimé ; l'application d'origine reste à moins de la désinstaller manuellement.",
        "de": "%1$@ Schritt%2$@. Jeder läuft nur nach Ihrer Bestätigung. Nichts wird gelöscht; die Original-App bleibt erhalten, bis Sie sie manuell deinstallieren.",
        "it": "%1$@ passaggi%2$@. Ciascuno viene eseguito solo quando confermi. Niente viene eliminato; l'app originale rimane a meno che non la disinstalli manualmente.",
    },
    "%@ unchecked": {
        "pt-PT": "%@ por verificar",
        "es": "%@ sin verificar",
    ...
    "%@ update%@ ready": {
        "pt-PT": "%1$@ atualização%2$@ pronta%2$@",
        "es": "%1$@ actualización%2$@ lista%2$@",
        "fr": "%1$@ mise à jour%2$@ prête%2$@",
        "de": "%1$@ Update%2$@ bereit",
        "it": "%1$@ aggiornament%2$@ pront%2$@",
    },
    "%@ · %@": {
        "pt-PT": "%1$@ · %2$@",
        "es": "%1$@ · %2$@",
    ...
    "%@%%": {"pt-PT": "%@%%", "es": "%@%%", "fr": "%@%%", "de": "%@%%", "it": "%@%%"},
    "%@.": {"pt-PT": "%@.", "es": "%@.", "fr": "%@.", "de": "%@.", "it": "%@."},
    "%@/100": {"pt-PT": "%@/100", "es": "%@/100", "fr": "%@/100", "de": "%@/100", "it": "%@/100"},
    "+%@ more": {
        "pt-PT": "+%@ mais",
        "es": "+%@ más",
        "fr": "+%@ de plus",
    ...
    "/100": {"pt-PT": "/100", "es": "/100", "fr": "/100", "de": "/100", "it": "/100"},
    ":": {"pt-PT": ":", "es": ":", "fr": " :", "de": ":", "it": ":"},
    "AI results for “%@” — %@ match%@": {
        "pt-PT": "Resultados da IA para “%1$@” — %2$@ correspondência%3$@",
        "es": "Resultados de IA para “%1$@” — %2$@ coincidencia%3$@",
        "fr": "Résultats IA pour « %1$@ » — %2$@ correspondance%3$@",
        "de": "KI-Ergebnisse für „%1$@“ — %2$@ Treffer%3$@",
        "it": "Risultati IA per “%1$@” — %2$@ corrispondenz%3$@",
    },
    "Apps we don't know yet (%@)": {
        "pt-PT": "Apps que ainda não conhecemos (%@)",
        "es": "Apps que aún no conocemos (%@)",
        "fr": "Applications que nous ne connaissons pas encore (%@)",
        "de": "Apps, die wir noch nicht kennen (%@)",
        "it": "App che non conosciamo ancora (%@)",
    },
    "Created %@": {
        "pt-PT": "Criado %@",
        "es": "Creado %@",
    ...
    "Higher than %@%% of your installed apps": {
        "pt-PT": "Superior a %@%% das tuas apps instaladas",
        "es": "Superior al %@%% de tus apps instaladas",
        "fr": "Supérieur à %@%% de vos applications installées",
        "de": "Höher als %@%% Ihrer installierten Apps",
        "it": "Superiore al %@%% delle tue app installate",
    },
    "Last used %@": {
        "pt-PT": "Último uso %@",
        "es": "Último uso %@",
    ...
    "Licensed to %@": {
        "pt-PT": "Licenciado a %@",
        "es": "Licenciado a %@",
        "fr": "Sous licence pour %@",
        "de": "Lizenziert für %@",
        "it": "Concesso in licenza a %@",
    },
    "Local scan • %@": {
        "pt-PT": "Análise local • %@",
        "es": "Análisis local • %@",
    ...
    "No public concerns recorded.": {
        "pt-PT": "Sem preocupações públicas registadas.",
        "es": "Sin preocupaciones públicas registradas.",
        "fr": "Aucune préoccupation publique enregistrée.",
        "de": "Keine öffentlichen Bedenken erfasst.",
        "it": "Nessuna preoccupazione pubblica registrata.",
    },
    "Page %@ of %@": {
        "pt-PT": "Página %1$@ de %2$@",
        "es": "Página %1$@ de %2$@",
    ...
    "Paste a URL or magnet": {
        "pt-PT": "Cola um URL ou magnet",
        "es": "Pega una URL o magnet",
        "fr": "Collez une URL ou un magnet",
        "de": "URL oder Magnet einfügen",
        "it": "Incolla un URL o magnet",
    },
    "Proposed by %@. Review each item — tap to open its homepage in a browser — then queue what you want.": {
        "pt-PT": "Proposto por %@. Revê cada item — toca para abrir a página inicial no browser — e adiciona o que quiseres à fila.",
        "es": "Propuesto por %@. Revisa cada elemento — toca para abrir su página de inicio en el navegador — y luego pon en cola lo que quieras.",
        "fr": "Proposé par %@. Examinez chaque élément — touchez pour ouvrir sa page d'accueil dans le navigateur — puis mettez en file d'attente ce que vous voulez.",
        "de": "Vorgeschlagen von %@. Prüfen Sie jeden Eintrag — tippen Sie, um die Homepage im Browser zu öffnen — und stellen Sie dann in die Warteschlange, was Sie möchten.",
        "it": "Proposto da %@. Esamina ogni elemento — tocca per aprirne la home page nel browser — quindi metti in coda ciò che vuoi.",
    },
    "Recording since %@.": {
        "pt-PT": "A gravar desde %@.",
        "es": "Grabando desde %@.",
        "fr": "Enregistrement depuis %@.",
        "de": "Aufzeichnung seit %@.",
        "it": "Registrazione dal %@.",
    },
    "Runs on %@. Typically 10–60 s.": {
        "pt-PT": "Corre em %@. Tipicamente 10–60 s.",
        "es": "Se ejecuta en %@. Normalmente 10–60 s.",
        "fr": "S'exécute sur %@. Généralement 10–60 s.",
        "de": "Läuft auf %@. Typischerweise 10–60 s.",
        "it": "Esegue su %@. Tipicamente 10–60 s.",
    },
    "Shareable (%@):": {
        "pt-PT": "Partilhável (%@):",
        "es": "Compartible (%@):",
    ...
    "Splynek’s Sovereignty catalog covers %@ paid apps and %@ alternatives.": {
        "pt-PT": "O catálogo de Soberania do Splynek cobre %1$@ apps pagas e %2$@ alternativas.",
        "es": "El catálogo de Soberanía de Splynek cubre %1$@ apps de pago y %2$@ alternativas.",
        "fr": "Le catalogue Souveraineté de Splynek couvre %1$@ applications payantes et %2$@ alternatives.",
        "de": "Splyneks Souveränitätskatalog deckt %1$@ kostenpflichtige Apps und %2$@ Alternativen ab.",
        "it": "Il catalogo Sovranità di Splynek copre %1$@ app a pagamento e %2$@ alternative.",
    },
    "Streaming-site URL.  yt-dlp %@ will fetch the best available format.": {
        "pt-PT": "URL de site de streaming. O yt-dlp %@ irá obter o melhor formato disponível.",
        "es": "URL de sitio de streaming. yt-dlp %@ obtendrá el mejor formato disponible.",
        "fr": "URL de site de streaming. yt-dlp %@ récupérera le meilleur format disponible.",
        "de": "Streaming-Site-URL. yt-dlp %@ ruft das beste verfügbare Format ab.",
        "it": "URL di sito di streaming. yt-dlp %@ recupererà il miglior formato disponibile.",
    },
    "Switched · +$%@/yr counted in your savings": {
        "pt-PT": "Trocado · +%@ USD/ano contabilizados nas tuas poupanças",
        "es": "Cambiado · +%@ USD/año contabilizados en tus ahorros",
        "fr": "Changé · +%@ USD/an comptés dans vos économies",
        "de": "Gewechselt · +%@ USD/Jahr in Ihren Einsparungen erfasst",
        "it": "Cambiato · +%@ USD/anno contati nei tuoi risparmi",
    },
    "This app is no longer at %@. Forget the entry to clean it up.": {
        "pt-PT": "Esta app já não está em %@. Esquece a entrada para limpar.",
        "es": "Esta app ya no está en %@. Olvida la entrada para limpiarla.",
        "fr": "Cette application n'est plus à %@. Oubliez l'entrée pour la nettoyer.",
        "de": "Diese App ist nicht mehr unter %@. Eintrag vergessen, um aufzuräumen.",
        "it": "Questa app non si trova più in %@. Dimentica la voce per ripulirla.",
    },
    "This download used **%@** alone. Splynek can stripe simultaneous byte-range requests across every interface you have — Ethernet + Wi-Fi, USB-C tether, iPhone hotspot, Tailscale. Add another and the next download finishes in a fraction of the time.": {
        "pt-PT": "Este download usou apenas **%@**. O Splynek pode distribuir pedidos byte-range simultâneos por cada interface que tens — Ethernet + Wi-Fi, USB-C, hotspot do iPhone, Tailscale. Adiciona outra e o próximo download termina numa fração do tempo.",
        "es": "Esta descarga usó solo **%@**. Splynek puede distribuir solicitudes byte-range simultáneas en cada interfaz que tienes — Ethernet + Wi-Fi, USB-C, hotspot del iPhone, Tailscale. Añade otra y la próxima descarga termina en una fracción del tiempo.",
        "fr": "Ce téléchargement a utilisé **%@** seul. Splynek peut répartir des requêtes byte-range simultanées sur chaque interface dont vous disposez — Ethernet + Wi-Fi, USB-C, partage iPhone, Tailscale. Ajoutez-en une autre et le prochain téléchargement se terminera en une fraction du temps.",
        "de": "Dieser Download nutzte ausschließlich **%@**. Splynek kann gleichzeitige Byte-Range-Anfragen über jede vorhandene Schnittstelle verteilen — Ethernet + Wi-Fi, USB-C, iPhone-Hotspot, Tailscale. Fügen Sie eine weitere hinzu und der nächste Download endet in einem Bruchteil der Zeit.",
        "it": "Questo download ha usato solo **%@**. Splynek può distribuire richieste byte-range simultanee su ogni interfaccia disponibile — Ethernet + Wi-Fi, USB-C, hotspot iPhone, Tailscale. Aggiungine un'altra e il prossimo download termina in una frazione del tempo.",
    },
    "Tip: drag any link onto the menu bar icon.": {
        "pt-PT": "Dica: arrasta qualquer link para o ícone da barra de menus.",
        "es": "Consejo: arrastra cualquier enlace al icono de la barra de menús.",
        "fr": "Astuce : faites glisser n'importe quel lien sur l'icône de la barre des menus.",
        "de": "Tipp: Ziehen Sie einen beliebigen Link auf das Menüleisten-Symbol.",
        "it": "Suggerimento: trascina qualsiasi link sull'icona della barra dei menu.",
    },
    "Today you've already used %@ from %@, past your %@ daily cap. Downloading anyway will clear today's cap for this host.": {
        "pt-PT": "Hoje já usaste %1$@ de %2$@, ultrapassando o teu limite diário de %3$@. Descarregar mesmo assim irá limpar o limite de hoje para este host.",
        "es": "Hoy ya has usado %1$@ de %2$@, superando tu límite diario de %3$@. Descargar de todos modos borrará el límite de hoy para este host.",
        "fr": "Aujourd'hui, vous avez déjà utilisé %1$@ depuis %2$@, dépassant votre limite quotidienne de %3$@. Télécharger quand même effacera la limite d'aujourd'hui pour cet hôte.",
        "de": "Heute haben Sie bereits %1$@ von %2$@ verbraucht und Ihr Tageslimit von %3$@ überschritten. Wenn Sie trotzdem herunterladen, wird das heutige Limit für diesen Host gelöscht.",
        "it": "Oggi hai già usato %1$@ da %2$@, superando il tuo limite giornaliero di %3$@. Scaricare comunque cancellerà il limite di oggi per questo host.",
    },
    "Trust Scan Report": {
        "pt-PT": "Relatório de Análise Trust",
        "es": "Informe de análisis Trust",
        "fr": "Rapport d'analyse Trust",
        "de": "Trust-Scan-Bericht",
        "it": "Rapporto di analisi Trust",
    },
    "Trust Scan Report (continued)": {
        "pt-PT": "Relatório de Análise Trust (continuação)",
        "es": "Informe de análisis Trust (continuación)",
        "fr": "Rapport d'analyse Trust (suite)",
        "de": "Trust-Scan-Bericht (Fortsetzung)",
        "it": "Rapporto di analisi Trust (continua)",
    },
    "Trust scan — top %@ most concerning": {
        "pt-PT": "Análise Trust — top %@ mais preocupantes",
        "es": "Análisis Trust — top %@ más preocupantes",
        "fr": "Analyse Trust — top %@ les plus préoccupantes",
        "de": "Trust-Scan — Top %@ mit größten Bedenken",
        "it": "Analisi Trust — top %@ più preoccupanti",
    },
    "Using %@%@": {
        "pt-PT": "A usar %1$@%2$@",
        "es": "Usando %1$@%2$@",
    ...
    "You've engaged with Trust Watcher %@ times. Splynek's evaluating a Trust+ subscription that adds: weekly Trust-catalog refreshes, acquisition radar (\"this app got bought by X — here's the privacy delta\"), one-click ToS history viewer.": {
        "pt-PT": "Já interagiste com o Trust Watcher %@ vezes. O Splynek está a avaliar uma subscrição Trust+ que adiciona: atualizações semanais do catálogo Trust, radar de aquisições (\"esta app foi comprada por X — eis o delta de privacidade\"), visualizador one-click do histórico de Termos.",
        "es": "Has interactuado con Trust Watcher %@ veces. Splynek está evaluando una suscripción Trust+ que añade: actualizaciones semanales del catálogo Trust, radar de adquisiciones (\"esta app fue comprada por X — aquí está el delta de privacidad\"), visor de historial de Términos con un clic.",
        "fr": "Vous avez interagi avec Trust Watcher %@ fois. Splynek évalue un abonnement Trust+ qui ajoute : actualisations hebdomadaires du catalogue Trust, radar d'acquisition (\"cette app a été rachetée par X — voici le delta de confidentialité\"), visionneuse d'historique des CGU en un clic.",
        "de": "Sie haben Trust Watcher %@ Mal verwendet. Splynek erwägt ein Trust+-Abo, das hinzufügt: wöchentliche Trust-Katalogaktualisierungen, Übernahme-Radar (\"diese App wurde von X gekauft — hier ist das Datenschutz-Delta\"), Ein-Klick-AGB-Verlaufsansicht.",
        "it": "Hai interagito con Trust Watcher %@ volte. Splynek sta valutando un abbonamento Trust+ che aggiunge: aggiornamenti settimanali del catalogo Trust, radar di acquisizioni (\"questa app è stata acquistata da X — ecco il delta di privacy\"), visualizzatore della cronologia dei Termini con un clic.",
    },
    "cap": {
        "pt-PT": "limite",
        "es": "límite",
