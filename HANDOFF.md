...
**Tests:** `swift run splynek-test` (**820 tests**, all green — was 740 before PRO-PLUS-IPHONE Sprint 1; +80 net across Trust Watcher pure-logic + CloudKit alert record round-trip + Sprint 2 scaffolds (Migrate plan, Concierge sequence, Geo-fence policy) + Sprint 2 part-2 (Migrate review list + Concierge sequence runner) + Sprint 3 (Concierge migrate-digest handler + engagement counters / Trust+ gate) + Sprint 4 (API tokens + 12 validator/store tests)) — `swift build` produces **0 warnings on clean rebuild** (verified on fresh DerivedData 2026-05-08)
**CLI:** `swift run splynek-cli version` (plus `sovereignty-dump` for catalog round-trip)

**Current version: v1.6.2 (Info.plist) / v1.5.3 (last pushed tag + uploaded DMG) — last work 2026-05-10 night (PRO-PLUS-IPHONE Sprint 4: Engagement viewer + Trust+ upsell card in SettingsView (consume the EngagementCounters foundation; privacy through transparency), L10n catalog +28 strings × 5 locales (closes 24/79 of the audit gap; remaining 55 are interpolated `%@` forms for Sprint 5), API tokens for external scripting (Aposta E developer power-up — persistent named tokens with read-only/read+write scopes, 12 tests).  4 commits 7f02266 → 088d8d1 + docs; 820/820 tests green.  Plus 2026-05-10 evening Sprint 3 (4 commits 85d6e4f → ec1e9d9 + docs bbd1f65) + 2026-05-10 morning Sprint 2 part-2 (4 commits 641dc70 → 9e1db78 + docs 6b4ca62) + 2026-05-09 evening Sprint 1 (5 commits 5e30f5c → fabf46e + docs 7a93885) + 2026-05-09 morning Settings decentralization (5 commits 57fb6cb → 2b3a87f) + 2026-05-08 design revolution + audit hardening + catalog growth strategy with 4,088-entry Homebrew Cask import; ~46 commits since `2efa8d0`, all pushed to `origin/rollup/2026-05-08` as backup; ~169 commits ahead of `origin/main` total).**
