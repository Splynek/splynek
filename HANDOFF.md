...
**Tests:** `swift run splynek-test` (579 tests, all green — was 552 before S4 foundation) — `swift build` produces **0 warnings on clean rebuild**
...
- ~~**G.** iOS Companion (Share Extension + Live Activity)~~ — **foundation shipped 2026-05-07.**  iOS app + Share Extension targets in `project.yml`; shared core under `iOS/Shared/` exposed as the `SplynekCompanionCore` SwiftPM library so `swift run splynek-test` exercises it; Bonjour discovery via `_splynek-fleet._tcp` + token-paste pairing + `/api/queue` submission + `/api/jobs` polling all working.  579 tests passing (+27 new).  The Mac side needed no changes — only consumes endpoints `FleetCoordinator` already exposes.  Phase 2 (Live Activity / QR pairing / CloudKit relay) deferred — see `IOS-COMPANION.md` for the architecture + phase-2 punch list.
