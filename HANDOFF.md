...
**Tests:** `swift run splynek-test` (**797 tests**, all green — was 740 before PRO-PLUS-IPHONE Sprint 1; +57 net across Trust Watcher pure-logic + CloudKit alert record round-trip + Sprint 2 scaffolds (Migrate plan, Concierge sequence, Geo-fence policy) + Sprint 2 part-2 (Migrate review list + Concierge sequence runner)) — `swift build` produces **0 warnings on clean rebuild** (verified on fresh DerivedData 2026-05-08)
**CLI:** `swift run splynek-cli version` (plus `sovereignty-dump` for catalog round-trip)

**Current version: v1.6.2 (Info.plist) / v1.5.3 (last pushed tag + uploaded DMG) — last work 2026-05-10 (PRO-PLUS-IPHONE Sprint 2 part-2: Sovereignty Migrate Wizard end-to-end (runner + SwiftUI sheet + persistent review list); Geo-fence iOS coordinator + Settings UI (CLLocationManager wrapper, "Use current location as home" + radius slider; coordinates never leave device); Watch app target skeleton (project.yml + scheme + minimal SwiftUI body with action buttons + Sovereignty score row + WKInterfaceDevice haptics; compile-verify pending watchOS SDK install); Concierge sequence runner + preview UI (actor wrapping MCPServer.Bridge dispatch + CheckedContinuation alert pattern).  4 commits 641dc70 → 9e1db78; 797/797 tests green; iOS xcodebuild SUCCEEDED.  Total Sprint 2 = 5 commits including the Sprint 2 part-1 scaffolds (9164418).  Plus 2026-05-09 evening Sprint 1 (5 commits 5e30f5c → fabf46e + docs 7a93885) + 2026-05-09 morning Settings decentralization (5 commits 57fb6cb → 2b3a87f) + 2026-05-08 design revolution + audit hardening + catalog growth strategy with 4,088-entry Homebrew Cask import; ~38 commits since `2efa8d0`, all pushed to `origin/rollup/2026-05-08` as backup; ~160 commits ahead of `origin/main` total).**
