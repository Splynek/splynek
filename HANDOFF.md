...
**Tests:** `swift run splynek-test` (611 tests, all green — was 552 before S4) — `swift build` produces **0 warnings on clean rebuild**
...
**Architectural state on `main`: next-release rollup ready** — single coherent forward release containing Concierge-as-Mac-Assistant, Verified Installer (osascript + SMJobBless paths), Fleet 2.0 LAN peer cache (with warm-cache, auto-join, household swarm token), **Bet S2 Unbreakable Resume** (path-flip pause/resume with sidecar continuity + curated mirror failover for Ubuntu/Debian/Fedora), **Bet S3 yt-dlp swallow** (DMG-only dispatch when user has yt-dlp installed; YouTube/Twitch/Instagram/TikTok/X/Vimeo/Bilibili route through it), **Bet S4 iPhone Companion foundation + phase 2 (2026-05-07)** — three iOS targets (SplynekCompanion app + SplynekShareExtension + SplynekCompanionWidgets), shared core exposed as `SplynekCompanionCore` SwiftPM library; pairing via Bonjour + **QR-code (`splynek://pair?...`) auto-fill** or token-paste fallback; Share Extension submits to `/api/queue`; **Live Activity for download progress on lock screen + Dynamic Island, mirrored into the Mac menu bar by macOS 26's Continuity Live-Activity passthrough — single ActivityKit implementation lights up both surfaces**; the macOS side gained one new method (`FleetCoordinator.iPhonePairingURLString()`) + a second QR card in Settings; CloudKit over-cellular relay + TestFlight gated on Apple v1.0 macOS clearance, **Bet S5 Browser Accelerator** (Chrome + Safari WebExtensions with `declarativeNetRequest` redirect; intercepts ≥50 MB downloads + HLS/DASH manifest URLs; HLSProxyServer rewrites segment URLs through localhost proxy + pre-buffers via BondedFetcher's multi-interface bonded Range fetch + LRU ring buffer), **Bet S6 File Witness** (Ed25519-signed download receipts with standalone verifier), **9 publisher patterns** (Mozilla/Apache/Debian/Ubuntu/Arch/GitHub/kernel.org/PyPI/Hugging Face) for digest auto-extraction, and the localization audit-script + CI guardrails (whole-file scan, scans both public + Pro repos).
...
- ~~**G.** iOS Companion (Share Extension + Live Activity)~~ — **foundation + phase 2 shipped 2026-05-07.**  Three iOS targets in `project.yml` (SplynekCompanion app + SplynekShareExtension + SplynekCompanionWidgets); shared core under `iOS/Shared/` exposed as `SplynekCompanionCore` SwiftPM library.  Phase 1: Bonjour discovery via `_splynek-fleet._tcp` + token-paste pairing + Share Extension URL submission via `/api/queue` + `/api/jobs` polling.  Phase 2 (same day): **Live Activity** for in-progress downloads on lock screen + Dynamic Island, with **macOS 26 mirroring it into the Mac menu bar via Continuity Live-Activity passthrough** (no Mac-side widget code required); **QR-code pairing** with Mac-side generator (`FleetCoordinator.iPhonePairingURLString()` + Settings → Web dashboard 2nd QR card) + iOS-side AVFoundation scanner that auto-fills + auto-submits PairingSheet.  611 tests passing (+59 over pre-S4).  Phase 3 (CloudKit over-cellular relay, TestFlight rollout) gated on Apple v1.0 macOS clearance — see `IOS-COMPANION.md`.
