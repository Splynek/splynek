...
**Tests:** `swift run splynek-test` (**740 tests**, all green — was 552 before S4; +188 net across S4 + 5-track sweep + 3-phase product expansion + 2026-05-08 design revolution + audit hardening + GitHub asset retry + catalog growth) — `swift build` produces **0 warnings on clean rebuild** (verified on fresh DerivedData 2026-05-08)
**CLI:** `swift run splynek-cli version` (plus `sovereignty-dump` for catalog round-trip)

**Current version: v1.6.2 (Info.plist) / v1.5.3 (last pushed tag + uploaded DMG) — last work 2026-05-09 (Settings decentralization: 6 cards moved out of Settings into their feature tabs — Trust weights → Confiança, Schedule + Watched folder → Fila, Swarm token + Security → Frota, Web dashboard + iPhone pairing QR → Agentes; brand footer restored in Sidebar.  Settings now 4 cross-cutting cards only: Pro license, browser helpers, local AI, background mode.  5 commits 57fb6cb → 2b3a87f; 740/740 tests still green.  Plus 2026-05-08 whole-app design revolution + audit hardening + catalog growth strategy with 4,088-entry Homebrew Cask import; ~26 commits since `2efa8d0`, all pushed to `origin/rollup/2026-05-08` as backup; ~145 commits ahead of `origin/main` total).**
