...
**Tests:** `swift run splynek-test` (**767 tests**, all green — was 740 before PRO-PLUS-IPHONE Sprint 1; +27 net from Trust Watcher pure-logic + CloudKit alert record round-trip tests added 2026-05-09) — `swift build` produces **0 warnings on clean rebuild** (verified on fresh DerivedData 2026-05-08)
**CLI:** `swift run splynek-cli version` (plus `sovereignty-dump` for catalog round-trip)

**Current version: v1.6.2 (Info.plist) / v1.5.3 (last pushed tag + uploaded DMG) — last work 2026-05-09 evening (PRO-PLUS-IPHONE Sprint 1: Trust Watcher (Pro marquee — daily ToS/Privacy Policy diff with severity-tagged alerts, 12 apps × 2 URLs seeded), Mac REST relay endpoints (pause-all/resume-all + sovereignty/trust/trust-watcher/history summaries), iOS App Intents (5 intents wired for Hey Siri), iOS Widget (small + medium home-screen), iOS Insights tab (Pro on iPhone — same Pro features as Mac surfaced on phone), CloudKit push notifications (Trust Watcher alerts → user's private DB → iPhone subscribed banner).  4 commits 5e30f5c → fabf46e; 767/767 tests green; iOS xcodebuild SUCCEEDED.  Plus 2026-05-09 morning Settings decentralization (5 commits 57fb6cb → 2b3a87f) + 2026-05-08 design revolution + audit hardening + catalog growth strategy with 4,088-entry Homebrew Cask import; ~30 commits since `2efa8d0`, all pushed to `origin/rollup/2026-05-08` as backup; ~150 commits ahead of `origin/main` total).**
