// Safari port of Extensions/Chrome/options.js — same logic, browser namespace.

const X = (typeof browser !== "undefined") ? browser : chrome;

//
// Edits the same `X.storage.sync` keys the background.js
...
function addHost(mode, inputEl) {
  ...
  X.storage.sync.get([KEYS.always, KEYS.optOut], (got) => {
...
function removeHost(host, mode) {
  const key = mode === "always" ? KEYS.always : KEYS.optOut;
  X.storage.sync.get(key, (got) => {
    const dict = { ...(got[key] || {}) };
    delete dict[host];
    X.storage.sync.set({ [key]: dict }, refresh);
  });
}
